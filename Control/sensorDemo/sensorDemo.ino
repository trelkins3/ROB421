...
void loop(){
  ...
  // Comment this out for blackbox testing, just set
  // incomingByte to = '1' or = '2'
  if(Serial.available() > 0){
    ...
    if(incomingByte == '0'){
      ...
      // Set back to 0 RPM after some time writing analog
      analogWrite(PWMoutPin,max(min(0 - 10*(int)(100 + _LeftEncoderVelocity) ,255),0));
  
      _LeftEncoderPrevTime = _LeftEncoderTime;
      _LeftEncoderPrevCount = _LeftEncoderTicks;
    }
    else if(incomingByte == '1'){ // top quadrant
      _LeftEncoderVelocity = 0.95*_LeftEncoderVelocity + 0.05*(60.0*1860.119*(_LeftEncoderTicks - _LeftEncoderPrevCount))/(_LeftEncoderTime - _LeftEncoderPrevTime);
      ...
      delay(500);
      // Set back to 0 RPM after some time
      analogWrite(PWMoutPin,max(min(0 - 10*(int)(100 + _LeftEncoderVelocity) ,255),0));
      ...
      _LeftEncoderPrevCount = _LeftEncoderTicks;
      delay(500);
      // Wait, then execute new serial read...
    }
  }
}
